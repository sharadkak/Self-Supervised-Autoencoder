...
from crumpets.torch.loss import MSELoss, L1Loss
...
from crumpets.workers import FCNWorker
...
exp = Experiment(' Experiment: Vanilla AE with Data augmentation')
...
def main(
    ...
    train = make_loader(
        ...
        gpu_augmentation=True, image_rng=AUGMENTATION_TRAIN, nsamples = nsamples
    )
    ...
    trainer = Trainer(
        # this metric is not right, find the appropriate one
        model, optimizer, loss, rank, MSELossMetric(),
        policy, None, train, None, outdir, snapshot_interval=4 if is_master else None
    )
