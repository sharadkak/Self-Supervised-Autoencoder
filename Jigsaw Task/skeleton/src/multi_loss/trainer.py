...
class Trainer(object):
    ...
    def train(self, num_epochs, start_epoch=0):
        ...
        try:
            ...
            accuracy = [k['top-1 acc'] for k in self.state['train_metric_values']]
            sigma_cl = [k['sigma_cl'] for k in self.state['train_metric_values']]
            sigma_ae = [k['sigma_ae'] for k in self.state['train_metric_values']]
            names = ['total_loss', 'ae_loss', 'cl_loss', 'accuracy', 'sigma_cl', 'sigma_ae']
            lists = [total_loss, ae_loss, cl_loss, accuracy, sigma_cl, sigma_ae]
