...
class SegNet(nn.Module):
    ...
    def __init__(self, num_classes ,n_init_features=3, drop_rate=0.5,
        ...
        decoder_filter_config = filter_config[::-1] + (n_init_features,)
        for i in range(0, 5):
    ...
    def forward(self, sample):
        ...
        feat = feat.sub(self.mean).div(self.std)
        feat = feat.type(torch.cuda.FloatTensor)
        sample['target_image'] = sample['target_image'].reshape((self.bs * 9, 3, 96, 96))
        sample['target_image'] = sample['target_image'].sub(self.mean).div(self.std)
        sample['target_image'] = sample['target_image'].type(torch.cuda.FloatTensor)
