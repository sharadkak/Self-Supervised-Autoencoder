...
def save_images(im, im_type, filename):

    # PIL can only work with uint8 datatype
    if im_type is 'input':
        im = im.view(9, 3, 96, 96)[0]
        im = im.cpu().numpy()
        dir_name = pt.join(ROOT, '../res/saved_test_input/')
        filename = '{}_im_{}.png'.format(im_type, filename)
    ...
    plt.imsave(dir_name + filename, im)
        ...
        for iteration, mini_batch in self.state['train_iter']:
            ...
            for sample in mini_batch:
                ...
                if iteration % int(n / 2) == 0:
                    ...
                    save_images(sample['image'][0], 'input', str(
                        iteration // self.state['epoch']))
                    save_images(output['output'][0], 'output',
                                str(iteration // self.state['epoch']))
                    save_images(output['target_image'][0], 'output', 
